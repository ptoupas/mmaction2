...
class ModelFeatureMaps:
            ...
            if hasattr(layer, "weight"):
                if hasattr(layer, "bias") and layer.bias is not None:
                    ...
                    self.layers[name] = [
                        ...
                        layer.weight.numel(),
                        (layer.weight.numel() * self.wb) / 1024**2,
                        list(layer.bias.shape),
                        layer.bias.numel(),
                        (layer.bias.numel() * self.wb) / 1024**2,
                    ]
                else:
                ...
                if isinstance(
                    layer, (torch.nn.Conv1d, torch.nn.Conv2d, torch.nn.Conv3d)
                ):
    ...
    def get_info(self):
        ...
        for n in self.activation.keys():
            ...
            if n in self.layers.keys():
                if ("conv" in n or "se_module" in n) and "bn" not in n:
                    macs = reduce((lambda x, y: x * y), self.layers[n][0]) * reduce(
                        (lambda x, y: x * y), list(self.activation[n][0].shape)[2:]
                    )
                ...
                if "bn" not in n and len(self.layers[n]) > 7:
                    print(
                        "Layer {} -> Input: {} - Output: {}, MBs {:.5f}\nFilters shape: {}. Params: {}, MBs: {:.5f}.\nBiases shape: {}. Params: {}, MBs: {:.5f}, MACs: {}.".format(
                            n,
                            list(self.activation[n][1].shape),
                            list(self.activation[n][0].shape),
                            (self.activation[n][0].numel() * self.wb) / 1024**2,
                            self.layers[n][0],
                            self.layers[n][1],
                            self.layers[n][2],
                            self.layers[n][3],
                    ...
                    self.layers_break_down[layers_num][n] = {
                        "input shape": list(self.activation[n][1].shape),
                        "output shape": list(self.activation[n][0].shape),
                        "intermediate feature map": (
                            self.activation[n][0].numel() * self.wb
                        )
                        / 1024**2,
                        "kernel shape": self.layers[n][0],
                        "kernel params": self.layers[n][1],
                        "kernel MBs": self.layers[n][2],
                        "bias shape": self.layers[n][3],
                        "bias params": self.layers[n][4],
                        "bias MBs": self.layers[n][5],
                        "MACs": macs,
                    }
                elif "bn" not in n and len(self.layers[n]) <= 7:
                    print(
                        "Layer {} -> Input: {} - Output: {}, MBs {:.5f}\nFilters shape: {}. Params: {}, MBs: {:.5f}, MACs: {}.".format(
                            n,
                            list(self.activation[n][1].shape),
                            list(self.activation[n][0].shape),
                            (self.activation[n][0].numel() * self.wb) / 1024**2,
                ...
                print(
                    "Layer {} -> Input: {} - Output: {}, MBs {:.5f}.".format(
                        n,
                        list(self.activation[n][1].shape),
                        list(self.activation[n][0].shape),
                        (self.activation[n][0].numel() * self.wb) / 1024**2,
        ...
        for key in self.layers_break_down.keys():
            ...
            bias_size = 0
            macs = 0
            prev_input_shape = self.layers_break_down[key][
                list(self.layers_break_down[key].keys())[0]
            ]["input shape"]
            for k in self.layers_break_down[key].keys():
                if (
                    "conv" in k or "pool" in k or "fc" in k or "bn" in k
                ) and "activate" not in k:
                    feature_maps_size += self.layers_break_down[key][k][
                        "intermediate feature map"
                    ]
                ...
                if (
                    ("conv" in k or "pool" in k or "fc" in k)
                    and "activate" not in k
                    and "bn" not in k
                ):
                    feature_maps_size_opt += self.layers_break_down[key][k][
                        "intermediate feature map"
                    ]
            print(
                "Layer {}. Input shape: {}. Feature maps size: {:.5f}({:.5f}) MBs. Kernel params: {}. Kernel size: {:.5f} MBs. Bias params: {}. Bias size: {:.5f} MBs. MACs: {}.".format(
                    key,
                    prev_input_shape,
                    feature_maps_size,
                    feature_maps_size_opt,
                    kernel_params,
                    kernel_size,
                    bias_params,
                    bias_size,
    ...
    def modeling(
        ...
        adds = adds_unrl_1 / (coarse_in * coarse_out) + adds_unrl_2 / (
    ...
    def get_conv_layers(self, file_name):
        ...
        csv_file = os.path.join(
            os.getcwd(), "fpga_modeling_reports", file_name + ".csv"
        )
        with open(csv_file, mode="w") as model_results:
            csv_writer = csv.writer(
                model_results, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL
            )
            ...
            csv_writer.writerow(
                [
                    ...
                    "On-Chip Memory(BRAM %)",
                    "DSPS %",
                    "Throughtput(outputs/sec)",
                    "Memory Bandwidth In(words/cycle)",
                    "Memory Bandwidth Out(words/cycle)",
                    "On-Chip Memory(KB)",
                    "On-Chip Memory(BRAM)",
                    "Memory Bandwidth In(GBs/sec)",
                    "Memory Bandwidth Out(GBs/sec)",
                    "Multipliers",
                    "Adders",
                    "DSPS",
                    "Throughtput(words/cycle)",
                    "Throughtput(GOps/sec)",
                ]
            )
            ...
            for n in self.activation.keys():
                ...
                if "conv" in name or "se_module.fc" in n:
                    ...
                    if kd == 1 and kh == 1 and kw == 1:
                        ...
                        pb = (
                            min((din * win * kh_minus), (win * hin * kd_minus))
                            + min((din * kw_minus), (win * kh_minus))
                            + cin
                        )
                            ...
                            for coarse_out in coarse_out_config:
                                for fine in fine_config:
                                    if (
                                        ...
                                        continue
                                    coarse_in_name = (
                                        str(coarse_in) if coarse_in == 1 else "Cin"
                                    )
                                    coarse_out_name = (
                                        str(coarse_out) if coarse_out == 1 else "Cout"
                                    )
                                    fine_name = str(fine)
                                    folding_name = "Coarse({}/{}) - Fine({})".format(
                                        coarse_in_name, coarse_out_name, fine_name
                                    )
...
def plot_graph(x, y, leg, name, type, model_name):
    ...
    mem_bw_dir = os.path.join(
        os.getcwd(), "fpga_modeling_reports", "graphs", model_name, "throughput_mem_bw"
    )
...
def performance_graphs(file_name="x3d_m", layer_to_plot=None):
    ...
    with open(csv_file, mode="r") as model_results:
        ...
        for i, row in enumerate(csv_reader):
            if "-" in row or (
                layer_to_plot is not None and row[cols["Layer"]] not in layer_to_plot
            ):
                continue

            if first_layer and (
                i == 0
                or (layer_to_plot is not None and row[cols["Layer"]] in layer_to_plot)
            ):
            ...
            if row[cols["Layer"]] == prev_layer:
                ...
                plot_graph(
                    throughput, [dsp_util], folding, prev_layer, "DSPS", file_name
                )
                plot_graph(
                    throughput,
                    [mem_bw_in, mem_bw_out],
                    folding,
                    prev_layer,
                    "Memory Bandwidth",
                    file_name,
...
def get_paretto(file_name="x3d_m"):
    ...
    csv_file_par = os.path.join(
        os.getcwd(), "fpga_modeling_reports", file_name + "_pareto.csv"
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--use_frames",
        action="store_true",
        help="whether to use video decoder or raw frame decoder",
    )
...
def main():
    ...
    feature_maps = ModelFeatureMaps(
        model=model, word_length=16, clock_freq=100, bram=624, dsp=1728
    )
