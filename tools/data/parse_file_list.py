...
def parse_directory(
    path, rgb_prefix="img_", flow_x_prefix="flow_x_", flow_y_prefix="flow_y_", level=1
):
    ...
    for i, frame_dir in enumerate(frame_dirs):
        total_num = count_files(frame_dir, (rgb_prefix, flow_x_prefix, flow_y_prefix))
        ...
        if num_x != num_y:
            raise ValueError(
                f"x and y direction have different number "
...
def parse_ucf101_splits(level, data_path="data/"):
    ...
    def line_to_map(line):
        ...
        if level == 1:
            ...
            video = osp.join(osp.basename(osp.dirname(video)), osp.basename(video))
